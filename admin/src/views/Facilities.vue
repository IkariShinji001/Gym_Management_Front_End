  ...
  <div>
    ...
    <div class="equipment-management">
      <div class="search-bar">
        <q-input class="input-search" v-model="searchQuery" label="Tìm kiếm thiết bị" filled />
        <q-btn class="btn-all" label="TẤT CẢ" @click="getAllFacilities" />
        <q-select class="search-bar-q-select" filled v-model="branchIdIsSelected" :options="branches"
          label="Chọn chi nhánh" map-options emit-value option-value="id" option-label="name"
          @update:model-value="filterFacilities" @click="updateBranches" />
        <q-btn class="btn-add" label="Thêm thiết bị" icon="add" color="primary" @click="OpenAddDialog" />
      </div>
        ...
        <table style="width: 100%">
          ...
          <tbody>
            ...
            <tr v-for="(facility, id) in filteredFacilities" :key="facility.id">
              ...
              <td>{{ facility.name }}</td>
              <td style="width: 15%">
                <img :src="facility.imageUrl" alt="Hình ảnh thiết bị" style="width: 150px; height: 100px" />
              </td>
              <td width="13%">{{ formatDate(facility.purchaseDate) }}</td>
              <td>
                <q-btn style="border-radius: 10px" icon="add" color="primary" @click="addMaintenance(facility.id)"
                  title="Thêm bảo trì" />
              </td>
              <td width="8%">
                <div class="q-pa-md">
      ...
      <q-dialog v-model="showFacilityDetails" persistent>
        <q-card class="dialog">
          ...
          <q-card-section>
            <q-form>
              <q-input v-model="facilityIsSelected.name" label="Tên thiết bị" readonly />
              <q-input v-model="facilityIsSelected.description" label="Mô tả" readonly />
              <q-input v-model="facilityIsSelected.branchId" label="Chi nhánh" readonly />
              <q-input type="date" v-model="facilityIsSelected.purchaseDate" label="Ngày mua" readonly />
              ...
              <q-input type="date" v-model="facilityIsSelected.warrantyEndDate" label="Ngày kết thúc bảo hành"
                readonly />
              <q-btn class="btn-cancel" icon="cancel" label="Thoát" flat @click="showFacilityDetails = false" />
            </q-form>
          </q-card-section>
        </q-card>
      </q-dialog>
        ...
        <q-card class="dialog">
          ...
          <q-card-section>
            <q-form @submit="addFacility">
              ...
              <q-file class="q-select" v-model="facility.imageUrl" label="chọn ảnh cho thiết bị" filled
        ...
        <q-card class="dialog">
          ...
          <q-card-section>
            <q-form @submit="
              ...
              <q-file v-model="facilityIsSelected.imageUrl" label="chọn ảnh cho thiết bị" filled
                :counter-label="file ? file.name : 'Chưa chọn tệp nào'" class="q-select"
